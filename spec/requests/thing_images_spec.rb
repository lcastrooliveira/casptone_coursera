...
RSpec.describe 'ThingImages', type: :request do
  ...
  describe 'ThingImage Authn policies' do
    ...
    let(:images) do
      (1..3).map { create_resource(images_path, :image, :created) }
    end
    let!(:unlinked_things) do
      (1..2).map { create_resource(things_path, :thing, :created) }
    end
    let(:orphan_image) { FactoryGirl.create(:image) }
    let(:thing_image_props) do
      { image_id: orphan_image.id }
    end
