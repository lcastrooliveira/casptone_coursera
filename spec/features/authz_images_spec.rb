...
RSpec.feature 'AuthzImages', type: :feature, js: true do
  ...
  let(:images) do
    FactoryGirl.create_list(:image, 3, :with_caption,
  ...
  shared_examples 'displays correct buttons for role' do |displayed, not_displ|
  ...
  shared_examples 'can create image' do
    it 'creates image' do
      ...
      within('sd-image-selector .image-list') do
        expect(page).to have_css('li a', text: /^#{image_props[:caption]}/,
                                         wait: 5)
      end
    end
  end
  ...
  context 'no image selected' do
    ...
    context 'unauthenticated user' do
      ...
      it_behaves_like 'displays correct buttons for role',
                      [],
                      ['Create Image', 'Clear Image', 'Update Image',
                       'Delete Image']
    end
  ...
  context 'images posted' do
    ...
    context 'user selects image' do
      before(:each) do
        find('div.image-list .id', text: image.id, visible: false)
          .find(:xpath, '..').click
      end
