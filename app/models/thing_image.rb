...
class ThingImage < ActiveRecord::Base
  ...
  def self.with_distance(origin, scope)
    scope.select('-1 as distance').with_position.each do |ti|
      ti.distance = ti.distance_from(origin)
    end
  end
