...
class ThingImagesController < ApplicationController
  ...
  helper ThingsHelper
  wrap_parameters :thing_image, include: %w[image_id thing_id priority]
  ...
  after_action :verify_authorized, except: %i[subjects]
  ...
  def linkable_things
    ...
    @things = ThingPolicy::Scope.new(current_user, @things)
  ...
  def create
    thing_image = ThingImage.new(thing_image_create_params
    ...
    if !thing
      ...
      if thing_image.save
        ...
        render json: { errors: @thing_image.errors.messages },
  ...
  def origin
    ...
    when params[:lng] && params[:lat]
      ...
      rails ActionController::ParameterMissing.new('an origin [lng/lat or address required]')
    end
  end
end
